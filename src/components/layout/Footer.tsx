'use client';

import Link from 'next/link';
import { cn } from '@/lib/utils';
import { getCurrentYear } from '@/lib/utils';
import Magnetic from '@/components/animations/magnetic';
import { getPrimarySocialLinks } from '@/data/social';
import { personalInfo } from '@/data/personal';

/**
 * Footer Component
 *
 * Site footer with copyright, tagline, and social links.
 * Common across all pages.
 *
 * @example
 * <Footer />
 */
export default function Footer() {
  const primaryLinks = getPrimarySocialLinks();
  const currentYear = getCurrentYear();

  return (
    <footer
      className={cn(
        'border-t border-gray-800',
        'bg-black text-white',
        'px-5 py-12 lg:px-8 lg:py-16'
      )}
      role="contentinfo"
    >
      <div className="mx-auto max-w-7xl">
        {/* Main Footer Content */}
        <div className="grid gap-8 md:grid-cols-3 md:gap-12">
          {/* Left - Tagline */}
          <div className="space-y-4">
            <h2 className="text-2xl font-bold text-teal-400">
              {personalInfo.name.split(' ')[0]}
            </h2>
            <p className="text-gray-400 max-w-xs">
              {personalInfo.tagline}
            </p>
          </div>

          {/* Center - Quick Links */}
          <nav className="space-y-4" aria-label="Footer navigation">
            <h3 className="text-sm font-semibold uppercase tracking-wider text-gray-500">
              Quick Links
            </h3>
            <ul className="space-y-2">
              {[
                { label: 'Home', href: '/' },
                { label: 'About', href: '/about' },
                { label: 'Work', href: '/work' },
                { label: 'Contact', href: '/contact' },
              ].map((link) => (
                <li key={link.href}>
                  <Magnetic>
                    <Link
                      href={link.href}
                      className={cn(
                        'inline-block text-gray-300 hover:text-teal-400',
                        'transition-colors duration-200',
                        'focus-visible:outline-none focus-visible:text-teal-400'
                      )}
                    >
                      {link.label}
                    </Link>
                  </Magnetic>
                </li>
              ))}
            </ul>
          </nav>

          {/* Right - Social Links */}
          <div className="space-y-4">
            <h3 className="text-sm font-semibold uppercase tracking-wider text-gray-500">
              Connect
            </h3>
            <ul className="flex gap-4">
              {primaryLinks.map((link) => (
                <li key={link.name}>
                  <Magnetic>
                    <a
                      href={link.url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className={cn(
                        'flex h-10 w-10 items-center justify-center',
                        'rounded-full bg-gray-800 text-gray-300',
                        'hover:bg-teal-400 hover:text-black',
                        'transition-colors duration-200',
                        'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-400 focus-visible:ring-offset-2 focus-visible:ring-offset-black'
                      )}
                      aria-label={`Visit ${link.name}`}
                    >
                      <SocialIcon name={link.icon} />
                    </a>
                  </Magnetic>
                </li>
              ))}
            </ul>
            <p className="text-sm text-gray-500">
              <a
                href={`mailto:${personalInfo.email}`}
                className="hover:text-teal-400 transition-colors"
              >
                {personalInfo.email}
              </a>
            </p>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-gray-800 pt-8 sm:flex-row">
          <p className="min-w-0 truncate text-sm text-gray-500">
            © {currentYear} {personalInfo.name}. All rights reserved.
          </p>
          <p className="shrink-0 text-xs text-gray-600">
            Built with Next.js, Tailwind CSS & GSAP
          </p>
        </div>
      </div>
    </footer>
  );
}

/**
 * Simple social icons component
 * Uses inline SVG for common social platforms
 */
function SocialIcon({ name }: { name: string }) {
  const icons: Record<string, React.ReactNode> = {
    github: (
      <svg
        className="h-5 w-5"
        fill="currentColor"
        viewBox="0 0 24 24"
        aria-hidden="true"
      >
        <path
          fillRule="evenodd"
          d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"
          clipRule="evenodd"
        />
      </svg>
    ),
    linkedin: (
      <svg
        className="h-5 w-5"
        fill="currentColor"
        viewBox="0 0 24 24"
        aria-hidden="true"
      >
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
  };

  return icons[name] || <span className="text-xs">{name[0]?.toUpperCase()}</span>;
}
