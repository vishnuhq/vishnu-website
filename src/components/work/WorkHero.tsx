...
// Line 2: "A look at what I've worked on" - each word/phrase separate
const taglineWords = ['A', 'look', 'at', 'what', "I've"];
const taglineHighlight = ['worked', 'on'];

// Size classes for responsive hero text
...
const animateLettersOnScroll = (containerRef: RefObject<HTMLDivElement | null>) => {
  ...
  const letterElements = lettersContainer?.querySelectorAll('.work-letter');
  ...
  return (
    <div className="flex flex-nowrap">
      {word.split('').map((letter, i) => (
        <div
          key={i}
          className={`work-letter font-semibold ${colorClass} ${sizeClass} leading-none`}
...
export function WorkHero() {
  ...
  return (
    <div ref={containerRef} className="h-screen overflow-hidden bg-background">
      <h1 className="sr-only">Vishnu Vardhan Putta - A look at what I've worked on</h1>
      <div aria-hidden="true" className="flex h-full flex-col items-center justify-center gap-4 px-4 sm:gap-6 sm:px-8">
        ...
        {/* Line 2: A look at what I've worked on (can wrap, "worked on" in teal) */}
        <div className="flex flex-wrap items-baseline justify-center gap-y-1">
          ...
          {taglineHighlight.map((word, wordIndex) => (
            <div key={`hl-${wordIndex}`} className="flex items-baseline">
              <LetterDisplay
                word={word}
                startIndex={getStartIndex(word)}
                sizeClass={taglineSize}
              ...
              />
              {wordIndex < taglineHighlight.length - 1 && <WordSpacer size="small" />}
            </div>
          ))}
        </div>
      </div>
